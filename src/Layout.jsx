...
const Layout = ({ user, logout }) => {
  ...
  // const [user, setUser] = useState(null) // Removed internal state for user
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const [loading, setLoading] = useState(false) // Keep loading for internal logic if needed
  ...
  const handleLogout = async () => {
    try {
      // Use the logout prop from App.jsx
      logout()
      navigate('/login')
    } catch (error) {
  ...
  // The user check is handled by ProtectedRoute in App.jsx, so we can assume user is present here.
  // If we need to check for user, we should use the prop.
  if (!user) {
